...
Inside endpoint handlers, calling `abort(404)` or similar is the simplest way
to return a non-200 response. The error response is determined by the router's
...
You can customize the actual error shape in the router's error handler (see
below).
...
| **422** Unprocessable Content | Request body fails schema validation         |
