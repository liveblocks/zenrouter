---
title: Authorization
description: Mandatory auth, request context, and public routes.
---

## The `authorize` function

Every `ZenRouter` requires an `authorize` function. It runs before every route
handler. See the [Request lifecycle](/docs/request-lifecycle). Return a falsy
...
Auth is **mandatory by design**. This ensures you can never accidentally expose
...
<Callout type="info">
  If most routes require auth but a few don't, split the public ones into a
  separate, isolated `ZenRouter` instance and then [compose them
  together](/docs/composing-routers).
</Callout>
...
See the [Request lifecycle](/docs/request-lifecycle) for a full overview of how
a request flows through the router.
