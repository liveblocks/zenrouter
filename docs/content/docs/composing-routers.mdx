...
Routes inside each router are still **fully qualified**. The relay prefix is
only used for dispatch. It does not strip or rewrite the URL. Each router
receives the original, unmodified request:
...
**complete, greppable URL path**. This is [intentional](/docs/principles#maintainable).
